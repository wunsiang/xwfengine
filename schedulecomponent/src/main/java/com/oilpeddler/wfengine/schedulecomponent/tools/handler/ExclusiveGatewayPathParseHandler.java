...
public class ExclusiveGatewayPathParseHandler extends AbstractPathParseHandler {
    ...
    public void parseOrPass(List<BaseElement> currentElementList,  Process process,List<BaseElement> addList,List<BaseElement> delList,String processInstanceId,String pdId) {
        ...
        nextPathParseHandler.parseOrPass(currentLevelResultList,process,addList,delList,processInstanceId,pdId);
    }
        ...
        if(currentExclusiveGateway.getIncomingFlows().size() > 1 && currentExclusiveGateway.getOutgoingFlows().size() == 1){
            ...
            for(SequenceFlow sequenceFlow : currentExclusiveGateway.getOutgoingFlows()){
                ...
                for(DataParam dataParam : sequenceFlow.getParamList()) {
                    WfProcessParamsRecordBO wfProcessParamsRecordBO = wfProcessParamsRecordService.getByEnginePpName(dataParam.getEnginePpName(),processInstanceId,pdId,dataParam.getTaskNo());
